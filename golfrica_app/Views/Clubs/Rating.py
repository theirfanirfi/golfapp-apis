...
class Rating(FlaskView):
    ...
    @route("rate_player/<int:player_id>", methods=['GET', 'POST'])
    def rate_player(self, player_id):
        ...
        if request.method == "GET":
            ...
            isPlayerRated, message, msg_type = self.rl.ratePlayer(user, player, data)
            self.response.update({"isRated": isPlayerRated, "message": message, "msg_type": msg_type})
            return jsonify(self.response)
