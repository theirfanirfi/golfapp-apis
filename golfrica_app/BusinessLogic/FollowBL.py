...
class FollowBL:
    ...
    def checkUserHasFollowedClub(self, user, club):
        follow = Follow.query.filter_by(
            follower_id=user.user_id, followed_id=club.club_id, is_club_followed=1
        )
        if follow.count() > 0:
            return follow.first()
        return False
    ...
    def checkUserHasFollowedPlayer(self, user, player):
        follow = Follow.query.filter_by(
            follower_id=user.user_id, followed_id=player.player_id, is_player_followed=1
        )
        if follow.count() > 0:
            return follow.first()
        return False
    ...
    def getClubFollowers(self, club_id):
        sql = text(
            "SELECT *, count(follows.f_id) as total_followers FROM follows WHERE followed_id = '"
            + str(club_id)
    ...
    def getClubFollowersForTab(self, user, club_id):
        sql = text(
            "SELECT follows.*, users.first_name, users.last_name, users.profile_image, users.user_id, "
    ...
    def getPlayerFollowersForTab(self, user, player_id):
        sql = text(
            "SELECT follows.*, users.first_name, users.last_name, users.profile_image, users.user_id, "
            "IF(follows.follower_id='"
            + str(user.user_id)
